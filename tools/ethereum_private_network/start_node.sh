...
if [[ $ROLE == "bootnode" ]]; then  # A normal full node without mining, acts as a bootnode
  ...
  geth --identity "$NODE" --ipcdisable --rpc --rpcaddr "0.0.0.0" --rpcport "8545" --rpccorsdomain "*" --datadir $datadir --port "30303" --rpcapi "db,eth,net,web3,personal" --networkid 1999
else
  ...
  if [[ $ROLE == "miner" ]]; then  # A full node runs in mining mode
    geth --identity "$NODE" --ipcdisable --bootnodes $bootnode_url --rpc --rpcaddr "0.0.0.0" --rpcport "8545" --rpccorsdomain "*" --datadir $datadir --port "30303" --rpcapi "db,eth,net,web3,personal" --networkid 1999 --mine --minerthreads 1
  else # A normal full node without mining by default
    geth --identity "$NODE" --ipcdisable --bootnodes $bootnode_url --rpc --rpcaddr "0.0.0.0" --rpcport "8545" --rpccorsdomain "*" --datadir $datadir --port "30303" --rpcapi "db,eth,net,web3,personal" --networkid 1999
  fi
fi
